...
behavior.nHealTimeToLiveUtilityMul = 0.5

function behavior:HealHealthUtilityFn(unitHero)
	local nUtility = 0
...
function behavior:TimeToLiveUtilityFn(unitHero)
...
behavior.nHealCostBonus = 10
behavior.nHealCostBonusCooldownThresholdMul = 4.0
function behavior:AbilityCostBonusFn(unitSelf, ability)
	local bDebugEchos = false
	...
	if nTimeToRegenMS < nCooldownMS * self.nHealCostBonusCooldownThresholdMul then
		return self.nHealCostBonus
	end
	
	return 0
end

behavior.unitHealTarget = nil
behavior.nHealTimeToLive = nil
	...
	if itemAstrolabe and itemAstrolabe:CanActivate() then
		...
		for key, hero in pairs(tTargets) do
			...
			if hero:GetUniqueID() ~= unitSelf:GetUniqueID() or core.GetCurrentBehaviorName(botBrain) ~= "HealAtWell" then
				...
				local nHealthUtility = self:HealHealthUtilityFn(hero) * self.nHealHealthUtilityMul
				local nTimeToLiveUtility = nil
				local nCurrentTimeToLive = nil
				nTimeToLiveUtility, nCurrentTimeToLive = self:TimeToLiveUtilityFn(hero)
				nTimeToLiveUtility = nTimeToLiveUtility * self.nHealTimeToLiveUtilityMul
		...
		if unitTarget then
			...
			self.unitHealTarget = unitTarget
			self.nHealTimeToLive = nTargetTimeToLive
		end		
	end
	
	if bDebugEchos then BotEcho(format("    abil: %s util: %d", sAbilName, nUtility)) end
	
	nUtility = nUtility * self.nHealUtilityMul
...
function behavior:Execute(botBrain)
	...
	local unitHealTarget = self.unitHealTarget
	local nHealTimeToLive = self.nHealTimeToLive
